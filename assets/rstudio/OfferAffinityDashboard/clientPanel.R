...
if(nchar(Sys.getenv('CP4D_HOSTNAME')) > 0 && nchar(Sys.getenv('CP4D_USERNAME')) > 0 && nchar(Sys.getenv('CP4D_PASSWORD')) > 0) {
  tryCatch({
    deploymentsResp = collectDeployments(Sys.getenv('CP4D_HOSTNAME'), Sys.getenv('CP4D_USERNAME'), Sys.getenv('CP4D_PASSWORD'), "offer_affinity_scoring_pipeline_function_deployment_tag")
...
clientServer <- function(input, output, session, sessionVars) {
  ...
  observeEvent(input$authBtn, {
    ...
    tryCatch({
      deploymentsResp = collectDeployments(input$hostname, input$username, input$password, "offer_affinity_scoring_pipeline_function_deployment_tag")
      serverVariables$deployments <- deploymentsResp$deployments
      serverVariables$token = deploymentsResp$token
    }, warning = function(w) {
